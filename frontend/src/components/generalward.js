...
retrieveDoctors(){
  axios.get("/doctors/get_generaldoctors").then(res =>{
    if(res.data.success){
      this.setState({
  ...
  retrieveNurses(){
    axios.get("/nurses/get_gennurses").then(res =>{
      if(res.data.success){
        this.setState({
      ...
      <div className="container">
        ...
        <div className="row">
            ...
            <div className="col-lg-12 mt-4 mb-2" > 
                <h4 style={{textAlign:'center'}}>General Ward Details</h4>
            </div>
        </div>
            <div className="row">
        ...
        <button type="button" class="btn btn-secondary" style={{width:'250px'}}><a href={`/view_genwarddoctors`} style={{textDecoration:'none', color:'white'}}>Doctors Schedule</a></button>
            </div>
        ...
        <button type="button" class="btn btn-secondary" style={{width:'250px'}}><a href={`/view_genwardnurse`} style={{textDecoration:'none', color:'white'}}>Nurses Schedule</a></button>
            </div>
        </div>
